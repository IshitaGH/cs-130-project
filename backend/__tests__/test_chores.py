...
def test_rotate_chore_daily():
    ...
    with app.app_context():
        ...
        assert chore.assignee_fkey == chore.rotation_order[1]  # Should switch to roommate2
...
def test_update_chore(client, test_data):
    ...
    response = client.put(f'/chores/{test_data["chore_id"]}', json=update_data, headers=headers)
    assert response.status_code == 200
