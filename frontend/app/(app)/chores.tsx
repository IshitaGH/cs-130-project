...
export default function ChoresScreen() {
  ...
  const [selectedRoommateId, setSelectedRoommateId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    const users = chores.filter(
      (chore) => chore.assigned_roommate?.id === userId,
    );
    const roommates = chores.filter(
      (chore) => chore.assigned_roommate?.id !== userId,
    );

    // Sort chores so completed ones come last
    setYourChores(
      [...users].sort((a, b) => Number(a.completed) - Number(b.completed)),
    );
    setRoommatesChores(
      [...roommates].sort((a, b) => Number(a.completed) - Number(b.completed)),
    );
  }, [chores]);
  ...
  const addOrUpdateChore = async () => {
    ...
    const effectiveRoommateId = isRecurring
      ? rotationOrder[0]
      : selectedRoommateId;
  ...
  const toggleComplete = async (id: number) => {
    ...
    if (!chore.is_task) {
      Toast.show({
        ...
        text2:
          'This chore is an ongoing responsibility and cannot be marked as complete.',
      });
      return;
    }
  ...
  const renderChoreRow = ({ item }: { item: Chore }) => {
    ...
    return (
      <View style={[styles.choreContainer, { marginBottom: 2 }]}>
        <Swipeable
          ref={swipeableRef}
        ...
        >
          <View style={[styles.choreRowWrapper, { backgroundColor: 'white' }]}>
            <TouchableOpacity
              activeOpacity={0.7}
              style={[
                styles.choreRow,
            ...
            >
              <View style={styles.avatar}>
                <Text style={styles.avatarText}>
                  {utils.getInitials(
                    item.assigned_roommate.first_name,
                    item.assigned_roommate.last_name,
                  )}
                </Text>
              </View>
              <View style={styles.choreInfo}>
                <View style={styles.choreNameRow}>
                  ...
                  <Text style={styles.choreDate}>
                    {item.recurrence !== 'none' ? 'Rotates' : 'Ends'}{' '}
              ...
              <TouchableOpacity
                onPress={() =>
                  item.assigned_roommate.id !== userId && remindChore(item)
                }
                          ...
                          <View key={roommateId} style={styles.rotationItem}>
                            <View
                              style={[
                                ...
                                roommateId === item.assigned_roommate.id &&
                                  styles.activeRotationAvatar,
                              ]}
                            >
                            ...
                            {item.rotation_order &&
                              index < item.rotation_order.length - 1 && (
                                <MaterialIcons
                                  name="arrow-forward"
                                  size={16}
                                  color="#666"
                  ...
                  <>
                    ...
                    <View style={styles.dropdown}>
                      {['daily', 'weekly', 'monthly'].map((option) => (
                        <TouchableOpacity
                          key={option}
                          onPress={() => setChoreRecurrence(option)}
                        >
                          <Text
                            style={[
                              styles.option,
                              choreRecurrence === option &&
                ...
                {isRecurring && (
                  <>
                    ...
                    <Text style={styles.sublabel}>
                      Tap roommates in the order they should rotate
                    </Text>
                    <ScrollView
                      horizontal={true}
                      showsHorizontalScrollIndicator={false}
                  ...
                  <>
                    ...
                    >
                      ...
                      <Text style={styles.dateText}>
                        {choreEndDate
                          ? new Date(choreEndDate).toLocaleDateString()
                          : 'Select Due Date'}
                      </Text>
                    </TouchableOpacity>
